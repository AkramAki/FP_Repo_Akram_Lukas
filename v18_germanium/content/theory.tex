...
For germanium detectors, the photoelectric effect dominates at low $\gamma$ energies and
is essential for achieving high full-energy detection probabilities and precise energy
measurements.
...
The full-energy detection probability quantifies the likelihood that an emitted photon
deposits its entire energy in the detector:
\begin{equation}
  \varepsilon_\text{FE} =
  \frac{N_\text{photopeak}}{N_\text{emitted}}.
\end{equation}

It depends on the detector dimensions, the extinction coefficient, and the photon energy.
For increasing energy, the probability of multiple Compton interactions without full
absorption rises, leading to a decreasing photopeak efficiency.

This behaviour explains why, despite a high total absorption probability, only a fraction
of events contribute to the photopeak.
