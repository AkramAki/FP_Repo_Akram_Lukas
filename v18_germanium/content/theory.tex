...
In this experiment, the detector is first calibrated in energy using a europium (Eu) source. 
Subsequently, the full-energy detection probability is determined, and characteristic spectral features 
of monoenergetic $\gamma$ radiation are analyzed. The relevant physical processes underlying these observations are 
discussed below.
...
at high photon energies, pair production becomes increasingly important, but only above the kinematic threshold
\begin{equation}
E_\gamma = 2 m_e c^2 \approx \qty{1.022}{\mega\electronvolt},
\end{equation}
where $m_e$ denotes the electron rest mass and $c$ is the speed of light in vacuum.
...
energy $E$ in the detector is observed. Using the Compton relation in \autoref{eq:compton}
...
\begin{equation}
  ...
  = \frac{3}{8}\,\sigma_{\mathrm{Th}}\,
    \frac{1}{m_e c^2 \epsilon^2}\left[
      2
      + \left(\frac{E}{E_\gamma - E}\right)^2
      \left(
        \frac{1}{\epsilon^2} + \frac{E_\gamma - E}{E_\gamma}
        - \frac{2}{\epsilon}\,\frac{E_\gamma - E}{E}
      \right)
    \right],
  \label{eq:kn_dsig_dE}
\end{equation}
with $\sigma_{\mathrm{Th}}$ the Thomson cross section, $m_e c^2$ the electron rest energy and 
