...
\subsection{Multimode Operation and Beat Frequencies}
The fundamental frequency spacing of the longitudinal modes is given by $\Delta f = c/(2L)$. For resonator lengths of the order of 
$\qty{0.6}{\meter}$ to $\qty{1.9}{\meter}$, this gives mode spacings in the order of $\sim\qty{70}{\mega\hertz}$ to $\sim\qty{250}{\mega\hertz}$, 
as also seen in the measurements. In contrast, the spectral width of the dominant red neon transition at $\qty{632.8}{\nano\meter}$ is about 
$\qty{1.5}{\giga\hertz}$. Since this transition width is much larger than the longitudinal mode spacing, many longitudinal modes lie inside the 
gain profile simultaneously. The He--Ne laser therefore has no physical constraint forcing single-mode operation in this configuration. 
Multimode operation is thus expected and the observation of multiple RF beat peaks confirms this.
Furthermore, the inverse relation between $\Delta f$ and $L$ is clearly visible: increasing the resonator length reduces the mode spacing. 
This behaviour is characteristic of a Fabry--Pérot cavity and matches the theoretical expression $\Delta f \propto 1/L$. 
The measured dependence therefore confirms that the beat frequencies observed on the spectrum analyser originate from the interference of simultaneously 
oscillating longitudinal cavity modes.
