\section{Theory} %% Maybe FT explanation or mention that gold atoms cannot be resolved because they are delocolized
%% Citations
\label{sec:theory}
...
\begin{equation*}
    I\propto\frac{U}{d}\exp(-Kd\sqrt{\bar{\phi}})\,,
\end{equation*}
where $U$ is the applied voltage between tip and sample, $d$ their distance, $K\approx1.025\cdot10^{10}\,\unit{\meter^{-1}\electronvolt^{-\frac{1}{2}}}$
and $\bar{\phi}$ the averaged work function. \cite{Lüth}
\subsection{Piezo crystals}
Piezo crystals are defined by their property to change their polarization when mechanical pressure is applied and inversely 
...
then polarizing them and cooling down again in order to ensure a long living polarization. \cite{Howland}
...
There are two methods of measuring when using a scanning tunelling microscope: Either one tries to keep the current constant, 
while varying the distance $d$ or the other way around. \cite{Lüth}
...
The other way around the response is larger when they have been used frequently. \cite{Howland}
On top of that the measurements might be disturbed by vibrations and thermal expansion 
as the stm is sensitive to small changes in distance \cite{Lüth}. 
...
\end{equation*}
where $S-V$ is the error of the current value compared to the setpoint. \cite{Heater}
...
It has a hexagonal crystal structure which is layered in 2d-plains that are bound by van-der-Waals interaction. 
